...
function cd_pb_admin() {
	...
	if ( isset($_POST['saveData']) ) {
		...
		$cd_pb[ 'friend' ] = $_POST[ 'cd_pb_friend' ];
	...
	<div class="wrap">
		...
		<form action="<?php echo site_url() . '/wp-admin/admin.php?page=cd-pb-admin' ?>" id="cd-pb-form" method="post">
			<p>
			<table id="" class="widefat link-group" style="width:75%">
				<thead>
					<tr class="header">
						<td colspan="2"><?php _e( 'Check here all fields you want to show in a bubble', 'cd_pb' ) ?></td>
						<td style=""><center><?php _e( 'As a link?', 'cd_pb' ) ?></center></td>
					</tr>
				...
				<?php foreach ( $all_fileds_ids as $field_obj => $field_data ) { ?>
					<tr>
						...
						<td scope="col" style="text-align:center">
							<input class="link" name="cd_pb_display[<?php echo $field_data->id?>][link]" type="checkbox" <?php if ( $cd_pb[ $field_data->id ]['link'] ) { ?>checked="checked" <?php } ?>value="yes" />
						</td>
				...
				<tfoot>
					<tr class="header">
						...
						<td style="text-align:center"><a href="#select_all"><?php _e( 'All', 'cd_pb' ) ?></a></td>
					</tr>
				</tfoot>
			</table>
			</p>
			...
			<table class="form-table">
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Do you want to display Mention and Private message links?', 'cd_pb' ) ?></th>
					<td>
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Do you want to display Add Friend button?', 'cd_pb' ) ?></th>
					<td>
						<input name="cd_pb_friend" type="radio" value="yes"<?php echo( ( 'yes' == $cd_pb[ 'friend' ] ) ? ' checked="checked"' : '' ); ?> /> <?php _e( 'Yes', 'cd_pb' ); ?><br>
						<input name="cd_pb_friend" type="radio" value="no"<?php echo( ( 'no' == $cd_pb[ 'friend' ] ) ? ' checked="checked"' : '' ); ?> /> <?php _e( 'No', 'cd_pb' ); ?><br>
					</td>
				</tr>
			</table>
			
			<p>
				<input class="button" type="submit" name="saveData" value="<?php _e( 'Save Selected Fields', 'cd_pb' ) ?>"/>
			</p>
		</form>
	</div>
	<?php
