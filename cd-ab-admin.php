...
class CD_AB_ADMIN_PAGE {
    ...
    function on_cd_ab_admin_users($cd_ab) {
        ...
        <table id="cd-ab-form" class="widefat link-group">
            <thead>
                <tr class="header">
                    <td colspan="2" style="border-bottom: 1px solid;"><?php _e('Check here all fields from users profiles you want to show in a bubble', 'cd_ab') ?></td>
                    <td style="border-bottom: 1px solid;"><center><?php _e('As a link?', 'cd_ab') ?></center></td>
                </tr>
            </thead>
            <tbody id="the-list">
            ...
            foreach ( $all_fields_ids as $field_obj => $field_data ) {
                ...
                <tr>
                    <td scope="col" width="10px">
                        <input name="cd_ab_display[<?php echo $field_data['id']?>][name]" type="checkbox" <?php if ( isset($cd_ab[ $field_data['id'] ]['name']) ) { ?>checked="checked" <?php } ?>value="<?php echo $field_data['name']?>" />
                    </td>
                    <td><?php echo $field_data['name'] . ' <span style="color:grey">&rarr; ' . $field_data['type'] .'</span>' ?></td>
                    <td scope="col" style="text-align:center">
                        <input class="link" name="cd_ab_display[<?php echo $field_data['id']?>][link]" type="checkbox" <?php if ( isset($cd_ab[ $field_data['id'] ]['link']) && $field_data['type'] != 'datebox') { ?>checked="checked" <?php } ?>value="yes" <?php if ( $field_data['type'] == 'datebox') { ?>disabled=true<?php } ?> />
                    </td>
            ...
            <tr>
                <td style="vertical-align:middle" colspan="2">
                    ...
                    if ( !bp_is_active( 'messages' ) ) {
                        echo '<br />&rarr; <span style="font-style: italic;">'.__('Messages Component is <strong>not</strong> activated. New Message link will <strong>not</strong> be visible.', 'cd_ab').'</span>';
                    }
                    ?>
                </td>
